...
const _keyApiKey = 'OKX_API_KEY';
const _keyApiSecret = 'OKX_API_SECRET';
const _keyPassphrase = 'OKX_PASSPHRASE';

/// App-scoped owner of the OKX session: stores credentials, opens and
/// closes the REST + WS connection, exposes the connection state.
class OkxSessionService {
  final FlutterSecureStorage _storage;
  final OkxAccountRepositoryFactory _accountFactory;
  ...
  final ValueNotifier<OkxAccountSession?> _session = ValueNotifier(null);
  ...
  ValueListenable<OkxAccountSession?> get session => _session;

  OkxSessionService({
    required FlutterSecureStorage storage,
    required OkxAccountRepositoryFactory accountFactory,
  ...
  Future<void> resync() async {
    ...
    // A long background stretch drifts the signing clock; realign it first or
    // every request below is rejected with 50102.
  ...
  /// Resyncs on returning to foreground even when [OkxAccountSession.startWs]
