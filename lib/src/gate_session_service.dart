...
const _keyApiKey = 'GATE_API_KEY';
const _keyApiSecret = 'GATE_API_SECRET';

/// App-scoped owner of the Gate session: stores credentials, opens and
/// closes the REST + WS connection, exposes the connection state.
class GateSessionService {
  final FlutterSecureStorage _storage;
  final GateAccountRepositoryFactory _accountFactory;
  ...
  final ValueNotifier<GateAccountSession?> _session = ValueNotifier(null);
  ...
  ValueListenable<GateAccountSession?> get session => _session;

  GateSessionService({
    required FlutterSecureStorage storage,
    required GateAccountRepositoryFactory accountFactory,
  ...
  /// Resyncs on returning to foreground even when [GateAccountSession.startWs]
